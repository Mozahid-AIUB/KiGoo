-- Ensures every active route has scheduled trips from now through 6pm
-- tomorrow, at the same every-4-hours slot times used by the other seeds.
-- Safe to re-run.
...
declare
  ...
  v_horizon timestamp := (current_date + 1) + time '18:00';
begin
    ...
    foreach v_direction in array array['to_varsity', 'from_varsity'] loop
      foreach v_dep_time in array v_slot_times loop
        ...
        for v_day in 0..1 loop
          ...
          if (v_trip_date + v_dep_time) <= v_horizon and (v_trip_date + v_dep_time) > now() then
